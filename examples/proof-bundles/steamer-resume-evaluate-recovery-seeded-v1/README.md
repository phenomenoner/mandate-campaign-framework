# Steamer resume-evaluate-recovery proof bundle (v1)

Canonical bounded proof pack for the Steamer blocked-`EVALUATE` recovery path (`advance-campaign.py --resume`).
...
2. forced queue-empty `EVALUATE` setup (deterministic fixture mutation; no secret/private data)
3. blocked `EVALUATE` step with wake reason `steamer_no_candidate_available`
4. explicit `--resume` invocation
5. recovery receipt `steamer_resume_recover_evaluate_to_explore`
6. post-resume inspect snapshot at `phase=EXPLORE`, `status=ACTIVE`
...
- `outputs/001_force_evaluate_queue_empty.txt` — deterministic fixture mutation receipt
- `outputs/advance/001_evaluate_to_blocked_no_candidate.json` — blocked step output
- `outputs/inspect_blocked.txt` / `outputs/inspect_blocked.json` — blocked-state inspect output
- `outputs/advance/002_blocked_resume_to_explore.json` — resume invocation + recovery step output
...
```bash
bash examples/proof-bundles/steamer-resume-evaluate-recovery-seeded-v1/replay.sh /tmp/mcf-resume-evaluate-proof-replay
```
...
- recovery receipt/event is `002_steamer_resume_recover_evaluate_to_explore`
- post-resume state is `phase=EXPLORE`, `status=ACTIVE`, `back_transitions_remaining=0`
