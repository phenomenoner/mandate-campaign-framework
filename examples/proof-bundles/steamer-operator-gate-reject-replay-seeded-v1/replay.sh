...
cp "$CAMPAIGN_DIR/CAMPAIGN_STATE.json" "$OUT_DIR/state_pre_replay.json"
python3 tools/inspect-campaign.py "$CAMPAIGN_DIR" > "$OUT_DIR/inspect_pre_replay.txt"
python3 tools/inspect-campaign.py "$CAMPAIGN_DIR" --json > "$OUT_DIR/inspect_pre_replay.json"

python3 tools/advance-campaign.py "$CAMPAIGN_DIR" --json \
  --replay-decision reject_shadow_review \
  --replay-note "shadow evidence rejected (seeded proof)" \
  > "$OUT_DIR/advance/008_operator_gate_replay_reject_to_closed.json"

cp "$CAMPAIGN_DIR/CAMPAIGN_STATE.json" "$OUT_DIR/state_post_replay.json"
python3 tools/inspect-campaign.py "$CAMPAIGN_DIR" > "$OUT_DIR/inspect_post_replay.txt"
python3 tools/inspect-campaign.py "$CAMPAIGN_DIR" --json > "$OUT_DIR/inspect_post_replay.json"
